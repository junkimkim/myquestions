...
import { getEffectiveTossSecretKey } from '@/lib/tossEffectiveKeys';

const TOSS_CONFIRM_URL = 'https://api.tosspayments.com/v1/payments/confirm';

export function getTossSecretKey() {
  const k = getEffectiveTossSecretKey();
...
export async function confirmTossPayment({ paymentKey, orderId, amount }) {
  ...
  if (!Number.isFinite(amountInt) || amountInt <= 0) {
    throw new Error(`confirmTossPayment: amount 가 유효하지 않습니다. (받은 값: ${amount})`);
  }
