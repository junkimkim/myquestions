...
/**
 * 보기 5개 블록 제목 — c_ecfd(객관식 어법) vs c_6dd(보기 문장) vs 지문 표현형
 * @param {string} typeId
 * @param {{ id: string, mcq_category?: string }[]} customTypes
 */
export function getGrammarPassageExprsFiveSectionLabel(typeId, customTypes) {
  if (typeId === GRAMMAR_SENTENCE_MCQ_TYPE_ID) return '객관식 어법 — 보기 문장 5개';
  if (isGrammarExprsOnlyType(typeId, customTypes)) return '객관식 어법 — 보기 5개';
  return '어법상 틀린 곳 찾기 — 지문상 표현 5개';
}

/**
 * 보기 5개 블록 안내 — c_ecfd와 c_6dd는 각각 다른 지시
 * @param {string} typeId
 * @param {{ id: string, mcq_category?: string }[]} customTypes
 */
export function getGrammarPassageExprsFiveHint(typeId, customTypes) {
  if (typeId === GRAMMAR_SENTENCE_MCQ_TYPE_ID) {
    return '문제 보기로 사용할 완전한 문장 5개를 입력해 주세요. 아래 오답 형태 칸에는 같은 번호에 맞춰 틀리게 고칠 부분을 입력합니다.';
  }
  if (isGrammarExprsOnlyType(typeId, customTypes)) {
    return '객관식 어법 문제의 선택지로 사용할 표현 5개를 입력해 주세요.';
  }
  return '지문에 실제로 나오는 밑줄 후보 표현(단어·구) 5개를 적어 주세요.';
}
